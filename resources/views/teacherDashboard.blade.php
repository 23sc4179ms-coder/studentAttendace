        ...
        <div class="row mt-3">
                    ...
                    <div class="card-body p-0">
                        <div class="list-group list-group-flush" id="teacherCourses">
                            @forelse($coursesList as $course)
                                ...
                                <button
                                    type="button"
                                    class="list-group-item list-group-item-action d-flex justify-content-between align-items-center btn-show-enrolled"
                                    data-course-id="{{ $course->id }}"
                                    data-url="{{ route('teacherDashboard.course.enrolled', $course->id) }}"
                                >
                                    <span>{{ $course->course_name }}</span>
                                    <span class="badge text-bg-secondary">{{ (int) ($course->enrolled_count ?? 0) }}</span>
                                </button>
                            @empty
            ...
            <div class="col-md-8 mt-3 mt-md-0">
                <div class="card shadow-sm">
                    ...
                    <div class="card-body" id="enrolledStudentsList">
                        <div class="text-muted">Click a course to view enrolled students.</div>
                    </div>
                </div>
            </div>
        </div>
